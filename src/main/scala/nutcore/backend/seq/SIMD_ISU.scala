...
class new_SIMD_ISU(implicit val p:NutCoreConfig)extends NutCoreModule with HasRegFileParameter{
    ...
    //def canforward(index:Int):Boolean = index == FuType.aluint || index == FuType.alu1int || index == FuType.lsuint || (if(Polaris_SIMDU_WAY_NUM != 0){index == FuType.simduint}else{false}) || (if(Polaris_SIMDU_WAY_NUM == 2){index == FuType.simdu1int}else{false})
...
//    val srcIsFloat = VecInit((0 to Issue_Num-1).map(i => src1IsFloat(i) || src2IsFloat(i) || src3IsFloat(i) || toFReg(i)))
    ...
    Debug("!!!SIMD_ISU_ready0!!! inReady %x in_valid %x out_fire %x\n", io.in(0).ready, io.in(0).valid, io.out(0).fire())
    Debug("!!!SIMD_ISU_ready1!!! inReady %x in_valid %x out_fire %x\n", io.in(1).ready, io.in(1).valid, io.out(1).fire())

...
class FDIV(val expWidth: Int, val sigWidth: Int) extends Module {
  ...
  val out_ready = io.out.ready
  val out_valid = io.out.valid
  ...
  // consts
  val len = sigWidth
  val lzc_width = log2Up(sigWidth+1)
...
//  val dNormAbsReg = RegEnable(dNormAbs, newReq | state(s_pre_0) | state(s_post_0))
  ...
  val rmReg = RegEnable(io.in.bits.rm, state(s_idle))
...
//  val sigCmpReg = RegEnable(sigCmp, state(s_pre_1))
  ...
  val zeroRes = (finalExp + (FloatPoint.expBias(expWidth)+sigWidth).U).head(1).asBool // inexact zero
  ...
  val special_sig = RegEnable(Mux(inv, 1.U << (sigWidth-2), 0.U(expWidth.W)), state(s_pre_1))
  ...
  val subResBits = finalExp + (FloatPoint.expBias(expWidth)+sigWidth).U // TODO sig cmp
  val normResBits = (sigWidth+1).U
  ...
  rounder.io.in := Mux(needShiftReg, qFinal(sigWidth, 2), qFinal(sigWidth-1, 1))
  ...
  val specialRmSig = Mux(noInf, Fill(sigWidth-1, true.B), 1.U((sigWidth - 1).W))

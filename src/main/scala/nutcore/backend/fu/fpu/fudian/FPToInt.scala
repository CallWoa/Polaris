...
class FPToInt(val expWidth: Int, val sigWidth: Int) extends Module {
  val io = IO(new Bundle() {
    val in = Flipped(new FPDataIn(expWidth, sigWidth, 1))
    val op = Input(UInt(2.W))
    val out = new IntDataOut(64)
  })
  val isSignedInt = io.op(0)
  val isLongInt = io.op(1)
  val fp_a = FloatPoint.fromUInt(io.in.src(0), expWidth, sigWidth)
  ...
  */
  ...
  val resultSign = raw_a.sign && isSignedInt
  val Result = Mux(resultSign, -ResultIntAbs, ResultIntAbs)

...
class IFU_inorder extends NutCoreModule with HasResetVector {
  ...
  // val npc = Mux(io.redirect.valid, io.redirect.target, Mux(crosslineJumpLatch, crosslineJumpTarget, snpc))
  ...
  Debug(pcUpdate,"[PCUP] pc=%x npc=%x redirecttarget=%x pnpc=%x snpc=%x bpuValid=%x bpuTarget=%x crosslineJump=%x crosslineJumpTarget=%x  \n",pc,npc,io.redirect.target,pnpc,snpc,bp1.io.out.valid,bp1.io.out.target,crosslineJump,crosslineJumpTarget)
...
class IFU_ooo extends NutCoreModule with HasResetVector {
  ...
  Debug("[pcUpdate] pcUpdate=%x io.redirect.valid=%x io.imem.req.fire=%x \n",pcUpdate,io.redirect.valid,io.imem.req.fire())
  ...
  // val bpuValid =  nlpvalid_latch
  // val bpuTarget = nlptarget_latch
  // val bpuBrIdx =  nlpbridx_latch
  ...
  Debug(nlp.io.in.pc.valid,"[BPUIN] npc=%x nlp.io.in.pc.bits=%x imem.fire=%x bpuValid=%x pnpc=%x snpc=%x \n",npc,nlp.io.in.pc.bits,io.imem.req.fire(),bpuValid,pnpc,snpc)
  Debug("[cosslineJump] state=%x crosslineJump=%x crosslineJumpTarget=%x  \n",state,crosslineJump,crosslineJumpTarget)
  ...
  when (pcUpdate) { 
   ...
   Debug(pcUpdate, "[IFUIN] pc:%x pcUpdate:%d npc:%x RedValid:%d RedTarget:%x LJL:%d LJTarget:%x LJ:%d snpc:%x bpValid:%d pnpc:%x \n",pc, pcUpdate, npc, io.redirect.valid, io.redirect.target,state === s_crosslineJump, crosslineJumpTarget, crosslineJump,snpc,nlp.io.out.valid,nlp.io.out.target)
        
  Debug(pcUpdate,"[PCUP] pc=%x npc=%x redirecttarget=%x pnpc=%x snpc=%x bpuValid=%x bpuTarget=%x crosslineJump=%x crosslineJumpTarget=%x  \n",pc,npc,io.redirect.target,pnpc,snpc,bpuValid,bpuTarget,crosslineJump,crosslineJumpTarget)
  ...
  Debug("[IFUOUT] io.out.bits.instr=%x io.out.bits.pc=%x io.out.bits.pnpc=%x io.out.bits.brIdx=%x io.out.bits.instValid=%x \n",io.out.bits.instr,io.out.bits.pc,io.out.bits.pnpc,io.out.bits.brIdx,io.out.bits.instValid)
...
class IFU_SIMD extends NutCoreModule with HasResetVector {
  ...
  val pc = RegInit(resetVector.U(VAddrBits.W))
  val pcUpdate = io.redirect.valid || io.imem.req.fire()    
  val snpc = Cat(pc(VAddrBits-1,3),0.U(3.W)) + 8.U
  val pcInstValid = RegInit("b1111".U)

  //BPU
  val bpu_SIMD = Module(new BPU_SIMD)
  val crosslineJump = bpu_SIMD.io.crosslineJump
  val crosslineJumpLatch = RegInit(false.B) 
  ...
  val pbrinfo = bpu_SIMD.io.brinfo
  // val npc = Mux(io.redirect.valid, io.redirect.target, Mux(crosslineJumpLatch, crosslineJumpTarget, snpc))
  val npc = Mux(io.redirect.valid, io.redirect.target, Mux(crosslineJumpLatch, crosslineJumpTarget, Mux(bpu_SIMD.io.out.valid, pnpc, snpc)))
  // Debug(p"[IFU NPC]npc = ${npc} pnpc=${pnpc} snpc = ${snpc} pc = ${pc} pc(2,1) = ${pc(2,1)} /n")
  val npcIsSeq = Mux(io.redirect.valid , false.B, Mux(crosslineJumpLatch, false.B, Mux(crosslineJump, true.B, Mux(bpu_SIMD.io.out.valid, false.B, true.B))))
  val npcInstValid = Wire(UInt(4.W))
  npcInstValid := Mux(crosslineJump && !(crosslineJumpLatch) && !io.redirect.valid, "b0001".U, InstValid(npc))

  // predicted branch position index
  val brinfo = Wire(UInt(4.W)) 
  brinfo := Cat(npcIsSeq, Mux(io.redirect.valid, 0.U, pbrinfo))
  val brIdx = Wire(UInt(4.W))
  ...
  io.imem.req.bits.apply(addr = Cat(pc(VAddrBits-1,1),0.U(1.W)), size = "b11".U, cmd = SimpleBusCmd.read, wdata = 0.U, wmask = 0.U, user = Cat(pcInstValid, brIdx & pcInstValid, Mux(crosslineJump, bpu_SIMD.io.out.target, npc), pc(VAddrBits-1, 0)))
  io.imem.req.valid := io.out.ready
  //TODO: add ctrlFlow.exceptionVec
  io.imem.resp.ready := io.out.ready || io.flushVec(0)

  io.out.bits := DontCare  //inst path only uses 32bit inst, get the right inst according to pc(2)

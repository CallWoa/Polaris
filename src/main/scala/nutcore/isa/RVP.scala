...
object RVPLSInstr extends HasInstrType {
  def LDLR   = BitPat("b000_????_?????_?????_011_?????_1110111")
  def LDLI   = BitPat("b001_????_?????_?????_011_?????_1110111")
  def LDSR   = BitPat("b010_????_?????_?????_011_?????_1110111")
  def LDSI   = BitPat("b011_????_?????_?????_011_?????_1110111")
  def STLR   = BitPat("b100_????_?????_?????_011_?????_1110111")
  def STLI   = BitPat("b101_????_?????_?????_011_?????_1110111")
  def STSR   = BitPat("b110_????_?????_?????_011_?????_1110111")
  def STSI   = BitPat("b111_????_?????_?????_011_?????_1110111")

  val table = Array(
    LDLR        -> List(InstrPLDR, FuType.lsu, SIMDUOpType.ldlr),
    //LDLI        -> List(InstrPLDI, FuType.lsu, SIMDUOpType.ldli),
    LDSR        -> List(InstrPLDR, FuType.lsu, SIMDUOpType.ldsr),
    //LDSI        -> List(InstrPLDI, FuType.lsu, SIMDUOpType.ldsi),
    STLR        -> List(InstrPSTR, FuType.lsu, SIMDUOpType.stlr),
    //STLI        -> List(InstrPSTI, FuType.lsu, SIMDUOpType.stli),
    STSR        -> List(InstrPSTR, FuType.lsu, SIMDUOpType.stsr),
    //STSI        -> List(InstrPSTI, FuType.lsu, SIMDUOpType.stsi)
  )
}